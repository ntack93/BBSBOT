...
class BBSBotApp:
    def __init__(self, master):
        ...
        self.public_message_history = self.load_public_message_history()  # Load public message history
    ...
    def get_chat_members(self):
        ...
        if public_trigger_match:
            ...
            # Ignore lines from Ultron itself
        ...
        if private_message_match:
            ...
            # Process direct messages (only if !nospam is OFF)
                ...
                # Public trigger commands:
                    ...
                    username = username_match.group(1) if username_match else "public_chat"
                    self.handle_doc_command(query, username)
                elif "!said" in clean_line:
                    username_match = re.match(r'From (.+?):', clean_line)
                    if username_match:
                        username = username_match.group(1)
                    else:
                        username = "unknown"
                    self.handle_said_command(username, clean_line)
    ...
    def store_public_message(self, username, message):
        """Store the public message for the given username."""
        ...
        self.save_public_message_history()  # Save the updated history to file

    def load_public_message_history(self):
        """Load public message history from a file."""
        if os.path.exists("public_message_history.json"):
            with open("public_message_history.json", "r") as file:
                return json.load(file)
        return {}

    def save_public_message_history(self):
        """Save public message history to a file."""
        with open("public_message_history.json", "w") as file:
            json.dump(self.public_message_history, file)
    ...
    def handle_said_command(self, sender, command_text):
        ...
        if len(parts) != 2:
            # Send usage as a private message to avoid public echo
            self.send_private_message(sender, "Usage: !said <username>")
            return
        ...
        self.send_full_message(f"[Ultron-said to {sender}] {command_text}")
        self.send_full_message(response)
